...
import com.aceproject.demo.scout.exception.AlreadyContractPlayerException;
...
import com.aceproject.demo.scout.model.YN;
...
public class ScoutReportingServiceImpl implements ScoutReportingService {
	...
	public void contract(int teamId, int slotNo) {
		...
		accountService.deductAp(teamId, CONTRACT_AP);
		...
		if (teamPlayer == null) {
			teamPlayer = new TeamPlayer(srSlot.getTeamId(), srSlot.getPlayerId());
	...
	public List<TeamSrSlotView> refresh(int teamId, ScoutReportOption option) {
		...
		List<Person> persons = personDao
				.list(players.stream().map(Player::getPersonId).distinct().collect(Collectors.toList()));
	...
	public List<TeamSrSlotView> apRefresh(int teamId, ScoutReportOption option) {
		...
		// ap 차감
		accountService.deductAp(teamId, option.getAp());
